...
if(isset($_REQUEST['fechaDesde']) && isset($_REQUEST['fechaHasta']))
    if($_REQUEST['fechaDesde']!="" && $_REQUEST['fechaHasta']!="")
            $periodo="cuentaproveedor.Fecha  BETWEEN '".$_REQUEST['fechaDesde']."' AND '".$_REQUEST['fechaHasta']."' AND";
if(isset($_REQUEST['fechaDesde']) && isset($_REQUEST['fechaHasta']))
    if($_REQUEST['fechaDesde']!="" && $_REQUEST['fechaHasta']!="")
...
array_push($nCampos,array("title"=>ucwords("Nombre"),"data"=>"Nombre","visible"=>1));
 ...
 DATE_FORMAT(cuentaproveedor.Fecha,'".$rango."') as FECHITA, 
...
cuentaproveedor.TipoComprobante IN('FA','FB','FC','FE' ,'FM') 
 AND cuentaproveedor.CodigoMovimiento > 0 ".$where." ".$puntodeventa."
