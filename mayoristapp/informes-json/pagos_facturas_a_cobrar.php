...
{ 
    ...
    if($tipo=="cliente"){
        $where.="  AND  proveedor.Codigo='".$idcliente."' ";
...
$sql="SELECT op_factura.id_op_factura as Codigo,
 CONCAT(`proveedor`.`Nombre`,' (',proveedor.Codigo,')') as NCLIENTE,
DATE_FORMAT(`op_factura`.`Fecha`,'%d-%m-%Y') AS FECHITA,
 ROUND(SUM( `op_factura`.`Importe`),".$decimales.") AS SALDO
 FROM   (`op_factura` `op_factura` INNER JOIN `cuentaproveedor` `cuentaproveedor` ON `op_factura`.`CodigoMovimiento`=`cuentaproveedor`.`CodigoMovimiento`) INNER JOIN `proveedor` `proveedor` ON `cuentaproveedor`.`Codigo`=`proveedor`.`Codigo`
 WHERE  ".$periodo."
  op_factura.TipoComprobante IN('FA', 'FB','FC','FM','ND') And op_factura.Anulado = 'No' And op_factura.Estado = 'N/Canc'  ".$where."
...
$_SESSION["titulo"]='Pagos de facturas a cobrar';
