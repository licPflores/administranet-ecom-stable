...
AND (ISNULL(stock.Codigogasto) OR stock.Codigogasto = 0)  ".$where." ".$puntodeventa." 
...
{stock.Comprobante}='FA'or{stock.Comprobante}='FB'  stock.Comprobante IN ('FA','FB') AND

({cuentaproveedor.CodSucursal} <> 0 ) AND  ({cuentaproveedor.FechaRegistro} >= #06-12-2018# and 
{cuentaproveedor.FechaRegistro} <= #06-12-2018#) and ({stock.TipoComp} = 'Compra' OR 
{stock.TipoComp} = 'Devol - Proveedor' OR {stock.TipoComp} = 'ND Anul NC' OR 
{stock.TipoComp} = 'Anul NC Devol' OR {stock.TipoComp} = 'Anul Compra')  
