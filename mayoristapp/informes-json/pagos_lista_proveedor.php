...
$sql = "SELECT cuentaproveedor.id_cuentaproveedor as Codigo,
 DATE_FORMAT(`cuentaproveedor`.`Fecha`,'%d-%m-%Y') AS FECHITA,
 ROUND(SUM(cuentaproveedor.ImportePago),".$decimales.") AS SALDO, 
 CONCAT(`proveedor`.`Nombre`,' (', `proveedor`.`Codigo`,')') AS NCLIENTE
 FROM   `cuentaproveedor` `cuentaproveedor` INNER JOIN `proveedor` `proveedor` ON `cuentaproveedor`.`Codigo`=`proveedor`.`Codigo`
 WHERE  ".$periodo." 
 cuentaproveedor.TipoComprobante = 'OP' And cuentaproveedor.CodigoMovimiento <> 0 And cuentaproveedor.Anulado ='No' ".$where."
...
$sql = "SELECT cuentaproveedor.id_cuentaproveedor as Codigo,
 DATE_FORMAT(`cuentaproveedor`.`Fecha`,'%m-%Y') AS FECHITA,
 ROUND(SUM(cuentaproveedor.ImportePago),".$decimales.") AS SALDO, 
 CONCAT(`proveedor`.`Nombre`,' (', `proveedor`.`Codigo`,')') AS NCLIENTE
 FROM   `cuentaproveedor` `cuentaproveedor` INNER JOIN `proveedor` `proveedor` ON `cuentaproveedor`.`Codigo`=`proveedor`.`Codigo`
 WHERE  ".$periodo." 
