...
$decimales = 2;
$where = " AND caja.tipo_comprobante NOT IN('MCAJ') ";
...
if (isset($_REQUEST['tipoResumen'])) {
    switch ($_REQUEST['tipoResumen']) {
        case "dia":
            ...
            $sql = " SELECT caja.id_caja AS Codigo,
                    ROUND(SUM(
                    IF(caja.ingreso IN(null,0),-1*caja.egreso,
                    IF(caja.egreso IN(NULL,0),caja.ingreso,'TuToMal'))
                    )," . $decimales . ") as SALDO,
                    CONCAT(caja_abm.nombre_caja,' (',caja_abm.id_caja,')') as NCLIENTE,
                    DATE_FORMAT(caja.fecha,'%d-%m-%Y') as FECHITA
                    FROM caja_abm caja_abm INNER JOIN caja caja ON caja_abm.id_caja=caja.id_caja_abm_origen
                    where " . $periodo . " 1=1 
                    $where
                    GROUP BY caja_abm.id_caja, FECHITA
                    ORDER BY FECHITA,caja_abm.id_caja";
        ...
        case "ano":
            ...
            $sql = " SELECT caja.id_caja AS Codigo,
                    ...
                    CONCAT(caja_abm.nombre_caja,' (',caja_abm.id_caja,')') as NCLIENTE,
                    DATE_FORMAT(caja.fecha,'%Y') as FECHITA
                    FROM caja_abm caja_abm INNER JOIN caja caja ON caja_abm.id_caja=caja.id_caja_abm_origen
                    where " . $periodo . " 1=1
                    $where
                    GROUP BY caja_abm.id_caja, FECHITA
        ...
        default:
            ...
            $sql = " SELECT caja.id_caja AS Codigo,
                    ...
                    CONCAT(caja_abm.nombre_caja,' (',caja_abm.id_caja,')') as NCLIENTE,
                    DATE_FORMAT(caja.fecha,'%m-%Y') as FECHITA
                    FROM caja_abm caja_abm INNER JOIN caja caja ON caja_abm.id_caja=caja.id_caja_abm_origen
                    where " . $periodo . " 1=1
                    $where
                    GROUP BY caja_abm.id_caja, FECHITA
...
if (!isset($sql))
    die("No tengo SQL para continuar");
...
$_SESSION["titulo"] = 'Estadistica Tabla Cruzada Caja';
