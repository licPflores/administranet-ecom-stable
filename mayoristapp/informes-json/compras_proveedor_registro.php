...
if(isset($_REQUEST['fechaDesde']) && isset($_REQUEST['fechaHasta']))
    if($_REQUEST['fechaDesde']!="" && $_REQUEST['fechaHasta']!="")
            $periodo="cuentaproveedor.FechaRegistro  BETWEEN '".$_REQUEST['fechaDesde']."' AND '".$_REQUEST['fechaHasta']."' AND";
if(isset($_REQUEST['fechaDesde']) && isset($_REQUEST['fechaHasta']))
    if($_REQUEST['fechaDesde']!="" && $_REQUEST['fechaHasta']!="")
        if(isset($_REQUEST['fechaDesdeDos']) && isset($_REQUEST['fechaHastaDos']))
            if($_REQUEST['fechaDesdeDos']!="" && $_REQUEST['fechaHastaDos']!="")
			{ 
		$periodo="(cuentaproveedor.FechaRegistro  BETWEEN '".$_REQUEST['fechaDesde']."' AND '".$_REQUEST['fechaHasta']."'  
		OR cuentaproveedor.FechaRegistro  BETWEEN '".$_REQUEST['fechaDesdeDos']."' AND '".$_REQUEST['fechaHastaDos']."' )  
		AND";
...
$where .= "\n AND  cuentacliente.CodViajante IN ('".implode("','",$articulos)."') ".PHP_EOL;
*/
/////////////////////////////////
$puntodeventa='';
$vectorsito= Array();
...
array_push($nCampos,array("title"=>ucwords("Nombre"),"data"=>"Nombre","visible"=>1));
 ...
 DATE_FORMAT(cuentaproveedor.FechaRegistro,'".$rango."') as FECHITA, 
 CONCAT(proveedor.Nombre,' (',proveedor.codigo,')') as NCLIENTE,
 ROUND(
SUM( cuentaproveedor.ImporteCompra) 
,".$decimales.")
AS SALDO
...
cuentaproveedor.TipoComprobante IN('FA','FB','FC','FE' ,'FM','NC','EB','ND') 
 AND cuentaproveedor.CodigoMovimiento <> 0 
 AND cuentaproveedor.anulado = 'No' 
 AND proveedor.Codigo <> 0 
  ".$where." ".$puntodeventa."
...
$sql= "  SELECT cuentaproveedor.id_cuentaproveedor as Codigo,
 ...
 ROUND(
SUM( 
IF(cuentaproveedor.Anulado ='Si' OR (cuentaproveedor.TipoComprobante='ND' AND cuentaproveedor.motivo_nd='Cheque rechazado'),0,
IF(cuentaproveedor.TipoComprobante='NC',cuentaproveedor.ImportePago*(-1),cuentaproveedor.ImporteCompra))
) 
,".$decimales.")
AS SALDO
FROM   `cuentaproveedor` `cuentaproveedor` 
...
// reporte 72
/* ({cuentaproveedor.CodSucursal} <> 0 ) AND  
(

 ({cuentaproveedor.TipoComprobante} = 'FA' OR {cuentaproveedor.TipoComprobante} = 'FB' OR {cuentaproveedor.TipoComprobante} = 'FC' OR {cuentaproveedor.TipoComprobante} = 'FE' OR {cuentaproveedor.TipoComprobante} = 'FM' OR {cuentaproveedor.TipoComprobante} = 'NC' OR {cuentaproveedor.TipoComprobante} = 'EB' OR {cuentaproveedor.TipoComprobante} = 'ND') AND ({cuentaproveedor.CodigoMovimiento} <> 0) 
 AND {cuentaproveedor.anulado} = 'No'  AND ({proveedor.Codigo} <> 0)
 ///////////////////////////////////////////////
 If {cuentaproveedor.Anulado} = "Si" or (({cuentaproveedor.TipoComprobante}="ND" AND{cuentaproveedor.motivo_nd}="Cheque rechazado")) then
formula=0
else
    if {cuentaproveedor.TipoComprobante}="NC"then
        formula = ({cuentaproveedor.ImportePago}*(-1)) 
    else
        formula={cuentaproveedor.ImporteCompra}
    end if
end if
//////////////////
IF(cuentaproveedor.Anulado ='Si' OR (cuentaproveedor.TipoComprobante='ND' AND cuentaproveedor.motivo_nd='Cheque rechazado'),0,
...
$_SESSION["titulo"]='Compras a Proveedor'; 
