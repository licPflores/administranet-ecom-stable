...
ROUND( SUM(
 ...
 AS SALDO
 FROM   (cuentacliente cuentacliente INNER JOIN viajantes viajantes ON cuentacliente.CodViajante=viajantes.CodViajante) INNER JOIN cliente cliente ON cuentacliente.Codigo=cliente.Codigo
WHERE  ".$periodo."
cuentacliente.TipoComprobante IN ('REC','FA','FB','FM','FE','FC') AND
cuentacliente.CondVenta IN ('Contado','-') AND
cuentacliente.Anulado ='No' ".$where." ".$puntodeventa." 
GROUP BY FECHITA, NCLIENTE
 ORDER BY FECHITA";
;
...
$_SESSION["titulo"]='Cobranza lista Vendedor';
