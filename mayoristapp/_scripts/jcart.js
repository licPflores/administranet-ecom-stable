			...
			$.ajaxSetup({
				...
				success: function(response) {
                    ...
                    $('#domicilio_entrega').on("change",selecRuta);
                        let cantDomicilos  = $('#domicilio_entrega').children('option').length;
                    console.log("Cantidad de domicilios: "+cantDomicilos);
                    // validar que hayan domicilios si no avisar.
                    if ($('#domicilio_entrega').children('option').length == 0) {
                        // si no tengo domicilios entonces aviso.    
                        console.log("El select está vacío. que domicilio tengo?",cantDomicilos);
                    }
                    // tengo opciones entonces selcciono una.
                    if ($('#domicilio_entrega').children('option').length > 0) {
                        $('#domicilio_entrega  option:eq(0)').prop('selected', true);
                    }
...
//             };
            ...
            /* funcion para la seleccion de rutas dinamicas*/
                var selecRuta = function(){
                    // debo buscar en el val del this este caracter "|" 

                    let controlZona = $(this).val();
                    console.log('mi valor de ruta:: ',controlZona);
                    if(controlZona !==null){
                        if (controlZona.indexOf("|") == -1) {
                            
                            alert("El domicilio seleccionado no posee zona, no se puede continuar.");
                            return;
                        }
                    }
                    if(controlZona==null){
...
//                                    console.log(response);
                                    ...
                                    if(sit<0){
                                        ...
                                        var cantidadOptions = $("#hoja_ruta option").length;
                                        listaRuta.attr("size", cantidadOptions);
                                    }else{
		...
		function add(form) {
			...
			$.ajax({
				...
				success: function(response) {
					...
					$('#jcart-buttons').remove();
                                         $('#domicilio_entrega').on("change",selecRuta);
                                         var opzion=$('#domicilio_entrega  option:eq(0)');
//                                        console.log(opzion);
        ...
        // muestra el panel de producto util para todos
        function jcartVerProductoInfo(obj){
            // console.log('quien soy:',$(this));
            let id= obj.attr('idProducto');
            console.log('id ',id);
            let panelDatosProducto = $('#jcartDivInfoProducto'+id);
            
            // console.log(panelDatosProducto);
             let botoni =obj.find('i');
			console.warn('botoni',botoni);
			let mostrar = botoni.hasClass('fa-angle-down');
			// //console.warn('mostrar',mostrar);
            // //let panelDatosProducto = $('.datos-cliente');
            if(mostrar==true){
			...
			panelDatosProducto.toggle(400);
        }
        

		function remove(link) {
			// Get the query string of the link that was clicked
            //console.log('soy eliminar producto');
            Swal.fire({
...
//                            
        ...
        //                            
               ...
               if(hayDomicilio!==undefined){
                    ...
                    /* selecciono el primero*/
                        console.log('agregue un producto aqui>>>>');
                    var opzion=$('#domicilio_entrega  option:eq(0)');
                        console.log(opzion);
                    opzion.prop("selected",true);
                    /* Logistica*/
		...
		$('.jcart').submit(function(e) {
            ...
            let buscarRapido,idProductoOculto;
            buscarRapido = $('#nombreBuscaRapido');
            idProductoOculto =$('#itemId');
            buscarRapido.val('');
            idProductoOculto.val('');
            buscarRapido.focus();
            Swal.fire({
                icon: 'success',
        ...
        container.delegate('.botonCheckout', 'click', function(e){

        // $('#jcart-checkout').on('click',function(e){
        //    var domicilio = $('#domicilio_entrega').val();
           var textoDomicilioSeleccionado = $('select#domicilio_entrega :selected').text();
        //    var tipoEntrega = $('#formaEntrega').val();
            var textoFormaEntrega = $('select#formaEntrega :selected').text();

            var cliente = $('.nombrecliente').text();
            console.log('que viende dentro de cliente?',$('.nombrecliente').text());
            var mensajeControl ="<h1>Resumen</h1>";
            mensajeControl +="<p>Cliente: "+cliente+"</p>";     
            mensajeControl +="<p>Domicilio: "+textoDomicilioSeleccionado+"</p>";
            mensajeControl +="<p>Forma de Entrega: "+textoFormaEntrega+"</p>";
            mensajeControl +="<p>¿Está seguro que desea generar el comprobante la compra?</p>";
            ...
            Swal.fire({
               ...
               if(accion.isConfirmed){
                    let jcartFormularioCarrito = $('#jcart-form-checkout');
                    jcartFormularioCarrito.submit();
                }
                if(accion.isCanceled){
                    return false;
                }
              });
              e.preventDefault();
        ...
        // container.delegate('#jcart-form-checkout','submit',function(event){
                    ...
                    // var cliente = $('.nombreCliente span').text();
                    //event.preventDefault();
                    // console.log('nombre dfel cliente a llamar',$(this));
                    //alert(cliente);
                    // hacer un llamada sweet alert. consultado
        ...
        // ver informacion
		container.delegate('.spanVerProductoInfo', 'click', function(e){
			
			// console.log('soy el span',$(this));
            
            jcartVerProductoInfo(($(this)));
            e.preventDefault();
		});
