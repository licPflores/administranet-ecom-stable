...
if (isset($_REQUEST['ajax'])) {
    ...
    if ($queInforme == "seleccion") {
        ...
        if (isset($_REQUEST["mostrarClientes"])) {
            $mostrarClientes = $_REQUEST["mostrarClientes"];
        }
        ...
        */
        ...
        $puntoVenta = $_REQUEST["puntoVenta"];
        $grafico = null;
        if (isset($_REQUEST["grafico"])) {
            $grafico = $_REQUEST["grafico"];
        }

        if (isset($_REQUEST["tipoInflacion"])) {
            $tipoInflacion = $_REQUEST["tipoInflacion"];
        } else {
            $tipoInflacion = null;
        }
        // articulos ensamblados en la venta
        if (isset($_REQUEST["artEnsambVenta"])) {
            $artEnsamblado = $_REQUEST["artEnsambVenta"];
        } else {
            $artEnsamblado = null;
        }
    ...
    switch ($queInforme) {
        ...
        case "uti":
            ...
            break;
        case "seleccion":
            $resultado = listado_seleccion($connV, $queTabla, $arrVendCargo,$mostrarClientes);
 ...
 */
function listado_seleccion($connV, $tabla = null, $arrVendCargo = null,$mostrarClientes)
{
    $sql = "";
    $lista = "";
    ...
    if($mostrarClientes == 'todos'){$verTodosClientes = 'Si';}
    ...
    // limito resultados a 10 
    // $sql .=" LIMIT 10";
    // if(debugSql==true){
    //     $sqlLog =file_put_contents('sql_utilidad'.date('Ymd').'.sql','seleccion:'.PHP_EOL.$sql.PHP_EOL,FILE_APPEND);
    // }
    ...
    while ($listado = mysqli_fetch_assoc($hacer)) {
        ...
        $arrLista[] = array(
            "label" => truncarCadena($listado["texto"]),
 ...
 // * funcion que filta los resultados excesivos en listados de filtros. 
...
function ventas_totales_todos(
    ...
    if ($rangoDoble == 1) {
        ...
        //        echo "</pre>";
        if(debugSql==true){
            $sqlLog =file_put_contents('sql_ventas_netas'.date('Ymd').'.sql','Traigo Periodos:'.PHP_EOL.$sqlPeriodo.PHP_EOL,FILE_APPEND);
        }
    /*
     * PERIODOS
     */
    ...
    //  echo "</pre>".PHP_EOL; 
    if(debugSql==true){
        $sqlLog =file_put_contents('sql_ventas_netas'.date('Ymd').'.sql','Traigo SQL VENTAS:'.PHP_EOL.$sqlTotal.PHP_EOL,FILE_APPEND);
    }
    ...
    if ($traigoArrayNc == 0) {
        ...
        //        echo "</pre><br>";
        if(debugSql==true){
            $sqlLog =file_put_contents('sql_ventas_netas'.date('Ymd').'.sql','Traigo SQL NC descuento en facturas :'.PHP_EOL.$sqlTotalNc.PHP_EOL,FILE_APPEND);
        }
    ...
    switch ($listarPor) {
        case 'cliente':
            ...
            if ($domCliente == null) {
                ...
                $tdomCliente = ",' | ',cli.Calle,' ',cli.NroCalle";
            }
...
function traer_valor_nc(
    ...
    //  print_r($sqlNC);
    if(debugSql==true){
        $sqlLog =file_put_contents('sql_ventas_netas'.date('Ymd').'.sql','Traigo SQL NC por valor :'.PHP_EOL.$sqlNC.PHP_EOL,FILE_APPEND);
    }
...
function utilidades_totales_todos(
    ...
    $sqlTotal = armar_sql_utilidad($tipo, $listarPor, $listaFiltro, $puntoVentaId, $periodo, $desde, $hasta);
            ...
            // echo "</pre>";
            if(debugSql==true){
                $sqlLog =file_put_contents('sql_utilidad'.date('Ymd').'.sql','Utilidad:'.PHP_EOL.$sqlTotal.PHP_EOL,FILE_APPEND);
            }
    ...
    if ($traigoArrayNc == 0) {
        ...
        //        echo "</pre>";
        if(debugSql==true){
            $sqlLog =file_put_contents('sql_utilidad'.date('Ymd').'.sql','Traigo NC:'.PHP_EOL.$sqlTotalNc.PHP_EOL,FILE_APPEND);
        }
...
function traer_valor_nc_utilidad($connV, $puntoVentaId = null, $desde = null, $hasta = null, $periodo, $filtrarPor = null)
{
    ...
    //print_r($sqlNC);
    if(debugSql==true){
        $sqlLog =file_put_contents('sql_utilidad'.date('Ymd').'.sql','Traigo Descuentos :'.PHP_EOL.$sqlNC.PHP_EOL,FILE_APPEND);
    }
