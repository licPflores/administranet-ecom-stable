...
function html_movil($notacredito,$vendedor){
        ...
        if(!empty($notacredito)){
            ...
            $muestro .='</tbody>';
                foreach($notacredito as $fact){
                    ...
                    $muestro .='    <td >'
                                . '<a  target="blank" href="ver_nota_credito_movil.php?codigomovimiento='.$fact->CodigoMovimiento.'&tipocomprobante='.$fact->TipoFact.'"  title="Visualizar comprobante" alt="Visualizar comprobante" mov="'. $fact->CodigoMovimiento.'" comprobante="'.$fact->TipoFact.'">' 
                                . '<i class="fa fa-file-pdf barrita fa-lg fa-fw fa-2x"></i>'
                                . '</a>';
                     $muestro .= '<a  href="relay-comprobante-a-mail.php?codMov='.$fact->CodigoMovimiento.'&tipocomprobante=1"  title="mandar Email" alt="enviar email" mov="'.$fact->CodigoMovimiento.'" comprobante="NC">
' 
...
if(isset($_POST['queryString'])) {
        ...
        if(strlen($queryString) >0) {
                ...
                $query = "SELECT 
                        ...
                        FROM comp_ped AS cc
                        WHERE 
                            cc.TipoComprobante IN('NCA','NCB','NCC','NCM','NCE')
...
//    echo "<pre>";
    ...
    if(isset($_SESSION["caminoDisp"])){
        ...
        if( $_REQUEST['campoBusca'] == 'TipoPedido'){
            if($_REQUEST['tipoFact'] == '-'){

                $consulta .="cc.TipoComprobante IN('NCA','NCB','NCC','NCM','NCE') ";
            }else{

                $tipoFact = $_REQUEST['tipoFact'];
                $consulta .= " cc.TipoComprobante='" . $tipoFact . "' ";
            }
        }else{
        ...
        if( $_REQUEST['campoBusca']=='NroComprobante' ){
   ...
   $sqlNc = "SELECT     
    ...
    $hacer = mysqli_query($connV,$sqlNc) or die('No puedo consultar el factura '.$sqlNc);
