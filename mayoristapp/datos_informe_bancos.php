...
CONCAT(banco.Nombre,' (',cuenta_banco.NroCuenta,')') as Cuentaa
 ...
 cuenta_banco.CodBanco = banco.CodBanco AND banco.cuentaabierta = 'Si'";
    ...
    $hacer = mysqli_query($conexion,$sql) or die('No puedo consultar la informacion de bancos'.$sql);
...
<head>
    ...
    <title>Bancos | administraNET</title>
...
$(document).ready(function(){
    ...
    $('#botonBuscar').click(function(){
        ...
        //datos.
        let cuentaBancaria = $('#cuentaBanco').val();
        let estadoCheque = $('#estadodecheque').val();
        let fechaCheque = $('#tfecha').val();
        ...
        if (tipoFiltro[0] == filtro) {
         ...
         if(validacion){
            ...
            $.ajax({
                    ...
                    url:'ajax/json_datos_informe_bancos.php',
                    method:'GET',
                    data:datos,
                    success: function(response){
                        $('#myTable').remove();
                     ...
                     if(informe == 'saldoBancos'){
                        ...
                        columnsConfig = [
                            ...
                            { data: 'comprobante', title: 'Nro Comprobante' },
                            { data: 'conciliado', title: 'Conciliado' },
                            { data: 'credito', title: 'Credito' },
                            { data: 'debito', title: 'Debito' },
                            { data: 'detalle', title: 'Detalle' },
                            { data: 'saldo', title: 'Saldo' },
                            { data: 'tipoComp', title: 'TipoComp' }
                        ]
                        ...
                        columnsConfig = [
                            { data: 'NroCheque', title: 'Nro Cheque' },
                            { data: 'banco', title: 'Banco' },
                            { data: 'cliente', title: 'Cliente' },
                            { data: 'estado', title: 'Estado' },
                            { data: 'fechaCobro', title: 'Fecha de Cobro' },
                            { data: 'fechaEmision', title: 'Fecha de Emision' },
                            { data: 'fechaVto', title: 'Fecha de Vto' },
                            { data: 'importe', title: 'Importe' },
                            { data: 'librador', title: 'Librador' },
...
<div id="wrapper">
        ...
        ?>
            ...
            <div class="paneles filtroInformes">
                <h1>Bancos<span><i id="parametrosInformes" class="fas fa-angle-up fa-lg fa-fw" aria-hidden="true"></i></span></h1>
                    ...
                    <div class="control">
                        <label for="agrupoPor" class="parametros">Informes:  </label>
                            <select name="agrupoPor" id="agrupoPor" required="required">
                                ...
                                <option value="saldoBancos">Saldo de bancos</option>
                                <option value="chequeTerceros">Lista de cheques de terceros</option>
                ...
                <div class="panelesBloqueInforme">
                    ...
                    <div class="titulo">
                        ...
                        <div id="buscaFecha" class="control w100p">
                            ...
                            <label for="fechaDesdeDos" class="parametros"><i class="fa fa-calendar fa-lg fa-fw" aria-hidden="true"></i> 
                            <input type="date" name="fechaDesdeDos" id="fechaDesdeDos"></label> al

                            <label for="fechaHastaDos" class="parametros"><i class="fa fa-calendar fa-lg fa-fw" aria-hidden="true"></i>
                             <input type="date" name="fechaHastaDos" id="fechaHastaDos"></label>
                        </div>
                        ...
                        <div class="panelesBloqueInforme" >      
                            <div class="titulo">Datos</div>
                                <div class="control" id="divCuenta" >
                                    <label for="cuentaBanco" class="parametros">Cuenta Bancaria:</label>		
                                    <select class="easyui-combobox" name="cuentaBanco" id="cuentaBanco">
                                    <option value="" selected="selected">Seleccionar una cuenta</option>
                                    <?php
                                    // Iterar sobre el array $campos
                                    foreach ($campos as $campo) {
                                        // Imprimir cada elemento del array como una opción en el select
                                        echo '<option value="' . $campo['codBanco'] . '">' . $campo['cuenta'] . '</option>';
                                    }
                                    ?>
                                    </select>
                ...
                <div class="panelesBloqueInforme" id="divEstado" >   
                <div class="control">
                <label for="estadodecheque" class="parametros">Estado:</label>
                <select name="estadodecheque" id="estadodecheque">
                <option value="Entregado">Entregado</option>
                <option value="Rechazado">Rechazado</option>
                <option value="Encartera">En cartera</option>
                <option value="Depositado" selected="">Depositado</option>
                <option value="En_nd_cliente">En nd cliente</option>
                <option value="En_nd_proveedor">En nd proveedor</option>
            ...
            <div class="panelesBloqueInforme" id="divFecha" >
                <div class="control">
                <label for="tfecha" class="parametros">Fechas:</label>
                    <select name="tfecha" id="tfecha">
                        <option value="emision" >Fecha Emision</option>
                        <option value="cobro">Fecha Cobro</option>
                        <option value="vto" selected="selected">Fecha Vencimiento</option>
                ...
                <div class="panelesBloqueInforme" id="divFiltro" >
...
<!-- <div class="paneles" id="contiene-tabla" style="overflow:auto"> -->
    ...
    <h1>Estadísticas de bancos <i id="expandir" class="fa fa-expand fa-lg fa-fw" title="expandir"></i> </h1>
